...
export default function NewWaitingRoom({ match, history }) {
    ...
    const changedRoomInfo = async (roomInfo) => {
        ...
        for await (const member of roomInfo.members) {
            ...
            const gameMember = {member, liar: {isCheckWord: false, isliar: false, order:false, count:0}, wordGame : {isCorrected: false, inputWord: ""}, alcoholRoulette: {location: 0, order: false}, rottenPlates: {order: false}}
